...
func TestRamp_RunCompletes(t *testing.T) {
	...
	eng := NewRamp(cfg, 3*time.Second, 5, 30, 1*time.Second)
	...
	if m.TotalRequests < 10 {
		t.Errorf("expected at least 10 requests (ramping up), got %d", m.TotalRequests)
	}
}
